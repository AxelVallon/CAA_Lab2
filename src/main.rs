 ...
 * - Update RSA key with new symetric key
 * - Update password with new symetric key
...
fn share_handler(account: &String, sym_key: &Vec<u8>) {
    ...
    if account.clone() == username_input {
        println!("{}","Operation failed. You can't share a password with yourself".color(Color::Red));
        return; //early return
    }
...
fn login_handler() {
    ...
    println!();
    let cred = Credentials::new(account_name_input.clone(), password_input); // password not in the memory anymore after verification
    // Credential verification and derivation of the symetric key.
    match cred.login() {
        Some(sym_key) => {
            println!("{}", "Login successful".color(Color::Green));
            println!("{}", "---------------------------------------------------".color(Color::DarkGray).bold());
            loop {
